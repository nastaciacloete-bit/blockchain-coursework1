...
namespace BlockchainAssignment
{
    ...
    public class Blockchain
    {
        ...
        //part 6.2 - replace idea of difficulty being number of zeros
        public int difficulty = 1000000;
        public int targetBlockTimeMs = 5000;
        public int minimumDifficulty = 100000;
        public int maximumDifficulty = 100000000;
        ...
        //adapted for part 5, for 6.1, 6.2 and again for 6.3
        public void AddBlock(string minerAddress, int threadCount, MiningSetting miningSetting)
        {
            List<Transaction> chosenTransactions = SelectTransactions(minerAddress, miningSetting);

            Block newBlock = new Block(GetLastBlock(), chosenTransactions, minerAddress, threadCount, difficulty, miningSetting.ToString());

            blocks.Add(newBlock);

            transactionPool = transactionPool.Except(chosenTransactions).ToList();

            AdjustDifficulty(newBlock.miningTimeMs);
        }

        //added for 6.3
        public List<Transaction> SelectTransactions(string minerAddress, MiningSetting miningSetting)
        {
            List<Transaction> chosenTransactions = new List<Transaction>();

            if (miningSetting == MiningSetting.Greedy)
            {
            ...
            else if (miningSetting == MiningSetting.Altruistic)
            {
                chosenTransactions = transactionPool
            ...
            else if (miningSetting == MiningSetting.Random)
            {
                Random random = new Random();

                chosenTransactions = transactionPool
                    .OrderBy(transaction => random.Next())
                    .Take(5)
                    .ToList();
            }
            else if (miningSetting == MiningSetting.AddressPreference)
            {
                chosenTransactions = transactionPool
                    .OrderByDescending(transaction =>
                        transaction.senderAddress == minerAddress ||
                        transaction.recipientAddress == minerAddress)
                    .ThenByDescending(transaction => transaction.fee)
                    .Take(5)
                    .ToList();
            }
        ...
        //part 6.2, controls difficulty based on mining times
        public void AdjustDifficulty(long miningTimeMs)
        {
            if (miningTimeMs < targetBlockTimeMs)
            {
                difficulty += 100000;
            }
            else if (miningTimeMs > targetBlockTimeMs)
            {
                difficulty -= 100000;
            }
        ...
        public string ValidateBlockchain()
        {
            for (int i = 1; i < blocks.Count; i++)
            {
                ...
                if (!currentBlock.HashMeetsDifficulty(currentBlock.hash)) // changed for 6.2
                {
