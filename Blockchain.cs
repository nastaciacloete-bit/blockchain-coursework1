...
namespace BlockchainAssignment
{
    public class Blockchain
    {
        ...
        //adapted for part 5, for 6.1, and again for 6.2
        public void AddBlock(string minerAddress, int threadCount)
        {
            List<Transaction> chosenTransactions = transactionPool.Take(5).ToList();

            Block newBlock = new Block(GetLastBlock(), chosenTransactions, minerAddress, threadCount, difficulty);

            blocks.Add(newBlock);

            transactionPool = transactionPool.Except(chosenTransactions).ToList();

            AdjustDifficulty(newBlock.miningTimeMs); //specific to 6.2
        }
